...
for tool in sfdisk losetup partx blkid mount umount mountpoint grep find mktemp; do
...
cleanup() {
    ...
    for dir in "$data_mount" "$root_b_mount" "$root_a_mount" "$boot_b_mount" "$boot_a_mount"; do
...
require cmp "$boot_a_mount/config.txt" "$root_a_mount/usr/lib/micropanel-touch/boot-selector-config.base"

require grep -Fq 'LABEL=MP_ROOT_A / ext4 ' "$root_a_mount/etc/fstab"
