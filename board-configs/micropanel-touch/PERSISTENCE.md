...
| `/data/xmproxy/etc/` | root:xmproxy, `0750`; files `0640` | xmproxy configuration: `xmpp-login.txt` (bot account, password, admin buddy, fallback), `manifest.json` (app controls), `xmproxy.env` | Seeded from `/opt/xmproxy/share/xmproxy/etc` by `xmproxy-seed.service` on every boot when a file is missing; existing files are never overwritten. A future launcher settings page writes `xmpp-login.txt` through a typed broker operation and restarts `xmproxysrv.service`. |
